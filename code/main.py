...
def main(args):
    ...
    if is_sim:
        workspace_limits = np.asarray([[-0.724, -0.224], [-0.25, 0.25], [-0.0001, 0.4]]) # Cols: min max, Rows: x y z (define workspace limits in robot coordinates)
        # workspace_limits = np.asarray([[-0.9, -0.1], [-0.1, 0.4], [-0.0001, 0.4]]) 
    else:
        workspace_limits = np.asarray([[0.3, 0.8], [-0.25, 0.25], [-0.255, -0.1]]) # Cols: min max, Rows: x y z (define workspace limits in robot coordinates)
    heightmap_resolution = args.heightmap_resolution # Meters per pixel of heightmap
    # random_seed = args.random_seed
    force_cpu = args.force_cpu
    # grasp & suction only or not
    no_gs = 0

    # ------------- Algorithm options -------------
    method = args.method # dueling DQN or Reactive
    # suction_rewards = args.suction_rewards if method == 'reinforcement' else None  # Use immediate rewards (from change detection) for suction?
    ...
    STEP = args.step
    TEST = args.test # The number of experiment test every 5 episode
    ...
    for episode in range(testing_episode if is_testing else training_episode):
        ...
        for step in range(STEP):                        
            ...
            ratio, color_num = 0.2, 12 # 8,12,16,18                                                
            depth18 = cv2.applyColorMap(cv2.convertScaleAbs(valid_depth_heightmap,alpha=255/ratio), color_num)            
            # plt.imshow(depth18)
            # plt.xticks([])
            # plt.yticks([])
            # plt.show()            
            ...
            if no_gs == 1 or objects_number == 1: #or trainer.iteration < 300:                
            ...
            elif no_gs == 0 and objects_number > 1:# and trainer.iteration >= 200:
                if method == 'reactive':
            ...
            if nonlocal_variables['primitive_action'] == 'grasp':
                ...
                label_mask1 = color_448.copy()
                cv2.polylines(label_mask,[poly],True,color=(225,225,0),thickness=2)
                cv2.putText(label_mask,str("Grasping"), (box_mask_cors_448[bestg_id[0],0,0],box_mask_cors_448[bestg_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(225,225,0),thickness=4)                
                cv2.rectangle(label_mask1,(object_boxes_448[bestg_id[0],0,0],object_boxes_448[bestg_id[0],0,1]),(object_boxes_448[bestg_id[0],1,0],object_boxes_448[bestg_id[0],1,1]),color=(225,225,0),thickness=2)
                cv2.putText(label_mask1,str("Grasping"), (object_boxes_448[bestg_id[0],0,0],object_boxes_448[bestg_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(225,225,0),thickness=4) 
                ...
                label_mask = color_448.copy()
                label_mask1 = color_448.copy()
                cv2.polylines(label_mask,[poly],True,color=(0,225,0),thickness=2)
                cv2.putText(label_mask,str("Sucking"),(box_mask_cors_448[bests_id[0],0,0],box_mask_cors_448[bests_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,225,0),thickness=4)                
                cv2.rectangle(label_mask1,(object_boxes_448[bests_id[0],0,0],object_boxes_448[bests_id[0],0,1]),(object_boxes_448[bests_id[0],1,0],object_boxes_448[bests_id[0],1,1]),color=(0,225,0),thickness=2)
                cv2.putText(label_mask1,str("Sucking"),(object_boxes_448[bests_id[0],0,0],object_boxes_448[bests_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,225,0),thickness=4) 
                ...
                label_mask = color_448.copy()
                label_mask1 = color_448.copy()
                cv2.polylines(label_mask, [poly_g], True,color=(225,225,0),thickness=2)                
                cv2.putText(label_mask,str("Grasping"),(box_mask_cors_448[bestgs_g_id[0],0,0],box_mask_cors_448[bestgs_g_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(225,225,0),thickness=4)
                cv2.polylines(label_mask,[poly_s],True,color=(0,225,0),thickness=2)
                cv2.putText(label_mask,str("Sucking"),(box_mask_cors_448[bestgs_s_id[0],0,0],box_mask_cors_448[bestgs_s_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,225,0),thickness=4)
                
                cv2.rectangle(label_mask1,(object_boxes_448[bestgs_g_id[0],0,0],object_boxes_448[bestgs_g_id[0],0,1]),(object_boxes_448[bestgs_g_id[0],1,0],object_boxes_448[bestgs_g_id[0],1,1]),color=(225,225,0), thickness=2)
                cv2.putText(label_mask1,str("Grasping"),(object_boxes_448[bestgs_g_id[0],0,0],object_boxes_448[bestgs_g_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(225,225,0),thickness=4) 
                cv2.rectangle(label_mask1,(object_boxes_448[bestgs_s_id[0],0,0],object_boxes_448[bestgs_s_id[0],0,1]),(object_boxes_448[bestgs_s_id[0],1,0],object_boxes_448[bestgs_s_id[0],1,1]),color=(0,225,0), thickness=2)
                cv2.putText(label_mask1,str("Sucking"),(object_boxes_448[bestgs_s_id[0],0,0],object_boxes_448[bestgs_s_id[0],0,1]),cv2.FONT_HERSHEY_SIMPLEX,1,(0,225,0),thickness=4)                            
                #label_mask = ndimage.rotate(label_mask, -90)                
                plt.figure(figsize=(20,30))
                plt.imshow(label_mask1)
                plt.xticks([])
                plt.yticks([])
            ...
            if 'prev_color_img' in locals():    
                ...
                if not is_testing:
                    ...
                    # print('label_value: %r' % (label_value))
                    ...
                    explore_prob = max(0.6 * np.power(0.9998, trainer.iteration),0.1) if explore_rate_decay else 0.6
            ...
            # Determine whether grasping or suction or grasping & suction should be executed based on network predictions
...
if __name__ == '__main__':
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description='Train robotic agents to learn how to plan complementary suction and grasping actions for manipulation with deep reinforcement learning in PyTorch.')

    # --------------- Setup options ---------------
    parser.add_argument('--is_sim', dest='is_sim', action='store_true', default=True,                                    help='run in simulation?')
    parser.add_argument('--obj_mesh_dir', dest='obj_mesh_dir', action='store', default='objects/success',                  help='directory containing 3D mesh files (.obj) of objects to be added to simulation')
    parser.add_argument('--num_obj', dest='num_obj', type=int, action='store', default=6,                                help='number of objects to add to simulation')
    ...
    # ------------- Algorithm options -------------
    parser.add_argument('--method', dest='method', action='store', default='reactive',                                 help='set to \'reactive\'  or \'reinforcement\'')
    ...
    parser.add_argument('--test', dest='test', type=int, action='store', default=5,                                       help='The number of experiment test every 5 episode')
