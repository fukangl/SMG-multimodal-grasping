...
class Logger():
    ...
    def __init__(self, continue_logging, logging_directory):
        ...
        self.color_heightmaps_directory_448 = os.path.join(self.base_directory, 'data', 'color-heightmaps-448')
        self.depth_heightmaps_directory_448 = os.path.join(self.base_directory, 'data', 'depth-heightmaps-448')
                
        self.color_masks_directory = os.path.join(self.base_directory, 'data', 'color-masks')
        self.depth_masks_directory = os.path.join(self.base_directory, 'data', 'depth-masks')
        
        self.color_action_masks_directory = os.path.join(self.base_directory, 'data', 'color-action-masks')
        ...
        if not os.path.exists(self.color_heightmaps_directory_448):
            os.makedirs(self.color_heightmaps_directory_448)
        if not os.path.exists(self.depth_heightmaps_directory_448):
            os.makedirs(self.depth_heightmaps_directory_448)      
        
        
        if not os.path.exists(self.color_masks_directory):
            os.makedirs(self.color_masks_directory)
        if not os.path.exists(self.depth_masks_directory):
            os.makedirs(self.depth_masks_directory)
        
        if not os.path.exists(self.color_action_masks_directory):
            os.makedirs(self.color_action_masks_directory)
    ...
    def save_heightmaps(self, iteration, color_heightmap, depth_heightmap, mode):
        ...
        #depth_heightmap = np.round(depth_heightmap).astype(np.uint16) # Save depth in 1e-5 meters
        cv2.imwrite(os.path.join(self.depth_heightmaps_directory, '%06d.%s.depth.png' % (iteration, mode)), depth_heightmap)
                      
    def save_heightmaps_448(self, iteration, color_heightmap, depth_heightmap, mode):
        color_heightmap = cv2.cvtColor(color_heightmap, cv2.COLOR_RGB2BGR)
        cv2.imwrite(os.path.join(self.color_heightmaps_directory_448, '%06d.%s.color.png' % (iteration, mode)), color_heightmap)
        #depth_heightmap = np.round(depth_heightmap * 10000).astype(np.uint16) # Save depth in 1e-4 meters
        cv2.imwrite(os.path.join(self.depth_heightmaps_directory_448, '%06d.%s.depth.png' % (iteration, mode)), depth_heightmap)
      
    
    def save_masks(self, iteration, num1, num2, color_masks, depth_masks, mode):
        color_masks = cv2.cvtColor(color_masks, cv2.COLOR_RGB2BGR)
        cv2.imwrite(os.path.join(self.color_masks_directory, '%06d.%s.%s.%s.color.png' % (iteration, mode, num1, num2)), color_masks)
        #depth_masks = np.round(depth_masks * 10000).astype(np.uint16) # Save depth in 1e-5 meters
        cv2.imwrite(os.path.join(self.depth_masks_directory, '%06d.%s.%s.%s.depth.png' % (iteration, mode, num1, num2)), depth_masks)
    
    def save_action_masks(self, iteration, color_masks, mode):
        color_masks = cv2.cvtColor(color_masks, cv2.COLOR_RGB2BGR)        
        cv2.imwrite(os.path.join(self.color_action_masks_directory, '%06d.%s.color.png' % (iteration, mode)), color_masks)
